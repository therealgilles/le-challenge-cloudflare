...
class Challenge {
  ...
  set({ acmeChallengePrefix, delay }, domain, challengePath, keyAuthorization, done) {
    const auth = crypto.createHash('sha256').update(keyAuthorization||'').digest('base64')
      .replace(/\+/g, '-')
      .replace(/\//g, '_')
      .replace(/=+$/g, '')
    const fqdn = `${acmeChallengePrefix}.${domain}`
    this.getZone(domain)
      .then(zone => {
  ...
  remove({ acmeChallengePrefix }, domain, key, done) {
    const fqdn = `${acmeChallengePrefix}.${domain}`
    this.getZone(domain)
